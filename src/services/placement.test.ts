...
describe('assignLevel()', () => {
  test.todo(
    'all-correct G1-2: returns highest passage level (18) - 1 = 17, clamped to 8 (G1-2 max)',
  );

  test.todo('all-wrong G1-2: returns grade-prior center (5) - 1 = 4, clamped within 2–8');

  test.todo(
    'mixed L2+L6 correct, L10+ wrong, G1-2: highest-60%-band is L6 → 6 - 1 = 5, within clamp 2–8',
  );
  ...
  test.todo(
    'edge: empty passageResults array → falls back to grade-prior center - 1 (bias-down minimum)',
  );
});

describe('deterministicShuffle()', () => {
  test.todo(
    'same seed produces same order on repeated calls (idempotent / retake reproducibility)',
  );
...
describe('abortPlacement() fallback formula', () => {
  ...
  test.todo(
    'too_easy at window max: result is clamped to window max (no overflow above grade band)',
  );
});
